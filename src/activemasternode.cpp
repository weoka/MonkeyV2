...
//
// Bootup the Masternode, look for a MONK collateral input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status == ACTIVE_MASTERNODE_INITIAL) {
        ...
        if (pmn != NULL) {
            pmn->Check();
            if (pmn->IsEnabled() && pmn->protocolVersion >= ActiveProtocol()) EnableHotColdMasterNode(pmn->vin, pmn->addr);
        }
    }
